...
public class ContentAct{
	...
	public String list(String queryStatus, Integer queryTypeId,
		...
		queryTitle = StringUtils.replaceEach(queryTitle, new String[]{"\'","<",">"}, new String[]{"","",""});
		queryTitle = StringUtils.trim(queryTitle);
		String queryInputUsername = RequestUtils.getQueryParam(request,"queryInputUsername");
		queryInputUsername = StringUtils.replaceEach(queryInputUsername, new String[]{"\'","<",">"}, new String[]{"","",""});
		...
		if(binary.isDateOverdue(user.getUsername()))
		{
			WebErrors errors1 = WebErrors.create(request);
			errors1.ErrorString("您的密码已过期，请到用户下修改密码！！！");
...
//			rights = user.getChannels(siteId);
	...
	public String save(Content bean, ContentExt ext, ContentTxt txt,ContentDoc doc,
		...
		String Realname = ext.getAuthor();
		Realname = StringUtils.replaceEach(Realname, new String[]{"\'","<",">"}, new String[]{"","",""});
		ext.setAuthor(Realname);
		Realname = ext.getDescription();
		Realname = StringUtils.replaceEach(Realname, new String[]{"\'","<",">"}, new String[]{"","",""});
		ext.setDescription(Realname);
		Realname = ext.getShortTitle();
		Realname = StringUtils.replaceEach(Realname, new String[]{"\'","<",">"}, new String[]{"","",""});
		ext.setShortTitle(Realname);
		Realname = ext.getTitle();
		Realname = StringUtils.replaceEach(Realname, new String[]{"\'","<",">"}, new String[]{"","",""});
		ext.setTitle(Realname);
		Realname = ext.getTitleColor();
		Realname = StringUtils.replaceEach(Realname, new String[]{"\'","<",">"}, new String[]{"","",""});
		if(Realname.length() >= 7)
			Realname = Realname.substring(0, 7);
		ext.setTitleColor(Realname);
		Realname = txt.getTxt();
		Realname = StringUtils.replaceEach(Realname, new String[]{"\'","<",">"}, new String[]{"","",""});
		txt.setTxt(Realname);
		if(tagArr != null)
		{
			...
			{
				tagArr[i] = StringUtils.replaceEach(tagArr[i], new String[]{"\'","<",">"}, new String[]{"","",""});
			}
		}
		if(picPaths != null)
		{
			...
			{
				picPaths[i] = StringUtils.replaceEach(picPaths[i], new String[]{"\'","<",">"}, new String[]{"","",""});
			}
		}
		if(picDescs != null)
		{
		for(int i = 0; i < picDescs.length;i++)
			{
				picDescs[i] = StringUtils.replaceEach(picDescs[i], new String[]{"\'","<",">"}, new String[]{"","",""});
			}
		}
	...
	public String check(String queryStatus, Integer queryTypeId,
		...
		for (Content bean : beans) {
			...
			cmsLogMng.operating(request, "content.log.update", "审核成功id=" + bean.getId()
					+ ";title=" + bean.getTitle());
		}
		return list(queryStatus, queryTypeId, queryTopLevel, queryRecommend,
				queryOrderBy, cid, pageNo, request, model);
	...
	public String reject(String queryStatus, Integer queryTypeId,
		...
		rejectOpinion = StringUtils.replaceEach(rejectOpinion, new String[]{"\'","<",">"}, new String[]{"","",""});
		CmsUser user = CmsUtils.getUser(request);
		Content[] beans = manager.reject(ids, user, rejectOpinion);
		for (Content bean : beans) {
			log.info("reject Content id={}", bean.getId());
			cmsLogMng.operating(request, "content.log.update", "审核退回id=" + bean.getId()
					+ ";title=" + bean.getTitle());
		}
		return list(queryStatus, queryTypeId, queryTopLevel, queryRecommend,
				queryOrderBy, cid, pageNo, request, model);
	...
	public String cycleList(Integer queryTypeId, Boolean queryTopLevel,
			...
			Integer pageNo, HttpServletRequest request, ModelMap model) throws Exception {
	...
	public String cycleRecycle(String queryStatus, Integer queryTypeId,
		...
		for (Content bean : beans) {
			log.info("delete Content id={}", bean.getId());
			cmsLogMng.operating(request, "content.log.update", "还原id="
					+ bean.getId() + ";title=" + bean.getTitle());
		}
	...
	public String cycleDelete(String queryStatus, Integer queryTypeId,
		...
		for (Content bean : beans) {
			log.info("delete Content id={}", bean.getId());
			cmsLogMng.operating(request, "content.log.delete", "完全删除id="
					+ bean.getId() + ";title=" + bean.getTitle());
		}
		return cycleList(queryTypeId, queryTopLevel, queryRecommend,
				queryOrderBy, cid, pageNo, request, model);
	...
	public String submitDocs(String[] docPaths, String[] docNames,String[] docFilenames,
			Integer channelId,Integer typeId,Integer modelId,Boolean draft,HttpServletRequest request,ModelMap model) throws Exception {
